...
namespace RemoteFileStorage.ViewModels
{
    class ItemsViewModel
    {
        ...
        public async Task UploadAsync(string path, string directory = "")
        {
            ...
            var extension = Path.GetExtension(path).Replace(".", "").ToUpper();
            ...
            directory = string.IsNullOrEmpty(directory) ? "" : directory + Path.DirectorySeparatorChar;
            extension = string.IsNullOrEmpty(extension) ? "" : extension + Path.DirectorySeparatorChar;
            filename = $"{directory}{extension}{filename}";
